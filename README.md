...
> 2026-09-05 真实供应商复验：本科代表任务和硕士代表任务均已使用真实模型完成10/10环并达到100%。硕士验收额外验证了进程重启、环6章节检查点和从失败任务断点续跑；修复后环7—环10继续收口，总计176,920 Token。博士代表任务与发布环境压力演练仍待完成。复验实际触发空响应、JSON截断、Token预算停止和字数不足，推动落地“结构化JSON关闭默认高思考、环6/7章节检查点、单章长度纠偏、相关度排序、DOCX真实段落化”等修复。控制流与恢复能力通过；由于首轮检索曾纳入偏题文献，新的相关度门禁尚待重新跑完整论文验证，学术质量仍是条件通过。详见 [真实模型本科十环验收报告](docs/真实模型本科十环验收报告_2026-08-26.md) 与 [WorkBuddy报告评审](docs/WorkBuddy深度分析报告评审_2026-08-26.md)。
...
- 环6/7采用逐章调用并保存章节级检查点；环6按剩余全文字数动态分配章节目标，短章最多四次增量补写，重试只继续缺失或不达标章节，空响应的已计费Token同样进入预算。
...
- 需要继续补充全文相关性/证据评测、提示词回归、博士真实长论文压力测试，以及发布环境的强杀/断网/Worker租约演练。
...
当前回归基线：**377 项 pytest 全部通过**（2026-09-05），另有 **31/31 条离线学术质量规则 case 通过**。M5 新增 6 项启动对账、持久化重启、Worker 租约与知识库故障测试；M6 增加真实全流程预检、学位预算和章节余量回归。
