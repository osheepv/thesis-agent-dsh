...
【当前分节的最小可信上下文】
{section_context}
【写作要求】只写当前 section_id；覆盖全部 claims；证据引用保留 [EVD-...] 内部标记；每个结果既要保留 [RES-...]，还必须在结果对象/表题/图题处定义 [[BOOKMARK:table_or_figure_id|显示标题]]；遇到相互矛盾证据必须如实呈现，不得擅自下确定结论；不得使用上下文之外的来源或结果。
