...
{agent_plan_block}
【要求】本次只输出“章节标题”指定的一个完整章节，不得输出其他章；输出 Markdown 正文（含 '## 节小节' 层级）；正文必须落在上方给出的当前章字数区间内，不得明显超长，不得用提纲、占位句或重复段落凑字数；内容论述必须有逻辑性、方法可复现、结论有证据支撑；引用文献时必须用 [L序号] 标注且仅限文献池内条目，**禁止引用池外/虚构任何文献**；使用实验结果时必须原样保留 [RES-*] 并定义对应 [[BOOKMARK:*|*]]；文献池为空时不要写任何『参考文献/张三等 提出』类表述；chapter_title 用'第N章 标题'格式；word_count 为实际可见字数估算。
