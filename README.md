...
基于 DSH 的学位论文全流程写作 Agent —— 后端一期最小可运行闭环。

> 一期范围：上传模板 → 选题(环1) → 大纲(环5) → 初稿(环6) → 导出 docx。
> 二期：前端、M7 文献数据源、M8 Guardrail、M9 知识库。二期需求与轮子选型见 [docs/二期需求与轮子选型分析.md](docs/二期需求与轮子选型分析.md)。
...
一期闭环（创建任务 → 环1 选题 → 环5 大纲 → 环6 撰写 → docx 生成校验导出）完整可运行，8 组 pytest 覆盖 FSM/执行体/闭环/契约层，`demo_run.py` 一键跑通。环1/5/6 当前为确定性 Mock 输出（尚无 LLM 接入），M7~M9 为预留桩。
