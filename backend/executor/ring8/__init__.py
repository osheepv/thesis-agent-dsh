# -*- coding: utf-8 -*-
"""环8 合规校验执行体（HITL 预留，本期仅留接口）。

M2 一期预告：合规校验属于 HITL 通过式网关环节（环2/4/8/10 之一），
需要人工确认政策/事实/格式合规，本期只定义骨架。

DSH 二期接入点：接入 Guardrail（M8）合规检查 + 人工确认流程。
"""
...
class Ring8ComplianceExecutor(RingExecutor):
    ...
    def execute(self, ctx: ExecContext) -> ExecResult:
        ...
        """
        raise NotImplementedError("环8 合规校验为 HITL 预留环节，本期未实现")
