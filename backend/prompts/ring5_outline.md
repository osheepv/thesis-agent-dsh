...
{argument_map_block}
【要求】章节结构遵循'提出问题→论证→解决→总结'闭环；必须覆盖论证图中的每个 section_id 和论断，不得新增与研究协议冲突的实验步骤或结果；每章要点（points）说明该章服务于哪个研究贡献；引用文献池时用 [L序号] 标注（仅限池内，禁止虚构）；输出平铺节点（level=1 章，level=2 节，level=3 要点），number 形如'第1章'/'1.1'/'1.1.1'；summary 为大纲整体说明。
